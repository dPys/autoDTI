...
if [[ $det_type != "DTI" ]] && [[ $det_type != "ODF" ]]; then
    echo -e "\nERROR: You must follow the -dt flag with either "DTI" or "ODF" when running FEED_openDTI.sh in order for deterministic tractography to run\n\n\n"
    touch "$output_dir"/"$PARTIC"/det_tractography_failed.txt
    exit 1
fi

##Set DTK PATH variable
export DSI_PATH="$openDTI_HOME"/DTI_TK/matrices
